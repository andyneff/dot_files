...
CUdevice_attribute = {
    ...
    # GPUDirect RDMA writes to the device do not need to be flushed for consumers within the scope indicated by the returned attribute. See CUGPUDirectRDMAWritesOrdering "for": e numerical values returned here.
...
# https://en.wikipedia.org/wiki/CUDA#Multiprocessor_architecture
...
class LibCuda:
    ...
    def cuInit(self, flags):
        ...
        return self.cuda.cuInit(flags)

    @cuda_api_call
    def cuDeviceGetCount(self, count):
        return self.cuda.cuDeviceGetCount(count)

    @cuda_api_call
    def cuDeviceGet(self, device, ordinal):
        return self.cuda.cuDeviceGet(device, ordinal)

    @cuda_api_call
    def cuDeviceGetName(self, name, len, dev):
        return self.cuda.cuDeviceGetName(name, len, dev)

    @cuda_api_call
    def cuDeviceComputeCapability(self, major, minor, dev):
        return self.cuda.cuDeviceComputeCapability(major, minor, dev)

    @cuda_api_call
    def cuDeviceGetAttribute(self, pi, attrib, dev):
        return self.cuda.cuDeviceGetAttribute(pi, attrib, dev)

    @cuda_api_call_warn
    def cuCtxCreate(self, pctx, flags, dev):
        try:
            result = self.cuda.cuCtxCreate_v2(pctx, flags, dev)
        except AttributeError:
            result = self.cuda.cuCtxCreate(pctx, flags, dev)
        return result

    @cuda_api_call_warn
    def cuMemGetInfo(self, free, total):
        try:
            result = self.cuda.cuMemGetInfo_v2(free, total)
        except AttributeError:
            result = self.cuda.cuMemGetInfo(free, total)
        return result

    @cuda_api_call
    def cuCtxDetach(self, ctx):
        return self.cuda.cuCtxDetach(ctx)


# Main function to get CUDA device specs
def get_cuda_device_specs(cuda) -> List[Dict[str, Any]]:
    ...
    cuda.cuDeviceGetCount(ctypes.byref(num_gpus))
    ...
    for i in range(num_gpus.value):
        ...
        if cuda.cuCtxCreate(ctypes.byref(context), 0, device) == CUDA_SUCCESS:
            free_mem = ctypes.c_size_t()
            total_mem = ctypes.c_size_t()

            cuda.cuMemGetInfo(ctypes.byref(free_mem), ctypes.byref(total_mem))

            spec["total_mem_mb"] = total_mem.value / 1024**2
            spec["free_mem_mb"] = free_mem.value / 1024**2

            spec["architecture"] = SEMVER_TO_ARCH.get(compute_capability,
                                                      "unknown")
...
if __name__ == '__main__':
    cuda = LibCuda()
    print(get_cuda_device_specs(cuda))
